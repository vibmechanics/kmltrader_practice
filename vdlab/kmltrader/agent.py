...
import numpy as np
from vdlab.kmltrader import utils

class Agent:
    '''
    투자 행동을 수행하고 투자금과 보유 주식을 관리하는 클래스
    environment: 환경 객체
    initial_balance: 초기 자본금
    min_trading_price: 최소 단일 매매 금액
    max_trading_price: 최대 단일 매매 금액
    balance: 현금 잔고
    num_stocks: 보유 주식 수
    ...
    def decide_trading_uint(self, confidence):
        '''
        매수 또는 매도할 주식 수 결정
        '''
    ...
    def act(self, action, confidence):
        ...
        if action == Agent.ACTION_BUY:
            # 매수할 단위를 판단
            trading_uint = self.decide_trading_uint(confidence)
            balance = (
                self.balance - curr_price *
                (1 + self.TRADING_CHARGE) * trading_uint
            )
            ...
            invest_amount = curr_price * (1 + self.TRADING_CHARGE) * trading_uint
            if invest_amount > 0:
                self.avg_buy_price = \
                    (self.avg_buy_price * self.num_stocks + curr_price * trading_uint) \
                        / (self.num_stocks + trading_uint) # 주당 매수 단가 갱신
                self.balance -= invest_amount # 보유 현금을 갱신
                self.num_stocks += trading_uint # 보유 주식 수를 갱신
